...
local NS = vim.api.nvim_create_namespace("eetcode_description")
...
local function inline(marks, row, line)
  delimited(marks, row, line, "%*%*[^%*]+%*%*", 2, "EetCodeBold")
  delimited(marks, row, line, "`[^`]+`", 1, "EetCodeInlineCode")
  delimited(marks, row, line, "%$[^%$]+%$", 1, "EetCodeMath")
end
...
local function render_md(text, lines, marks, prefix, tight)
  ...
  local function close_code()
    if code_start and #lines > code_start then
      table.insert(marks, { code_start, 0, {
        ...
        hl_group = "EetCodeCodeBlock", hl_eol = true,
      } })
    end
  ...
  for _, raw_line in ipairs(vim.split(clean_html(text), "\n", { plain = true })) do
    ...
    if url and url ~= "" then
      ...
      table.insert(marks, { #lines - 1, 0, { end_col = #label, hl_group = "EetCodeFold" } })
      images[#lines - 1] = url
    ...
    if heading then
      ...
      table.insert(marks, { #lines - 1, 0, { end_col = #lines[#lines], hl_group = "EetCodeSection" } })
      seen, pending = true, true
    elseif line:match("^%-%-%-+$") then
      ...
      for _, span in ipairs(spans) do
        table.insert(marks, { row, span.from, { end_col = span.to, hl_group = "EetCodeLink" } })
...
local function render_tags(tags, lines, marks)
  ...
  for i, tag in ipairs(tags) do
    ...
    local from = #text
    text = text .. tag.name
    table.insert(spans, { from, #text, "EetCodeTag" })
    if tag.count then
      text = text .. " " .. tag.count
    end
  end
  ...
  table.insert(marks, { row, 0, { end_col = #text, hl_group = "EetCodeMuted" } })
  for _, span in ipairs(spans) do
    table.insert(marks, { row, span[1], { end_col = span[2], hl_group = "EetCodeTag" } })
  end
end
...
function M.render(buf, problem, meta, sections, opts)
  ...
  table.insert(marks, { #lines - 1, 0, { end_col = #lines[#lines], hl_group = "EetCodeTitle" } })
  table.insert(lines, "")
  ...
  for _, section in ipairs(sections) do
    if section.kind == "md" then
      ...
      table.insert(marks, { #lines - 1, 0, { end_col = #header, hl_group = "EetCodeFold" } })
  ...
  if #footer > 0 then
    ...
    for _, entry in ipairs(footer) do
      ...
      table.insert(lines, label)
      table.insert(marks, { #lines - 1, 0, { end_col = #label, hl_group = "EetCodeMuted" } })
