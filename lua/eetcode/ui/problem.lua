...
local description = require("eetcode.ui.description")
local hl = require("eetcode.ui.highlight")
local lang_info = require("eetcode.lang")
local progress = require("eetcode.progress")
local results = require("eetcode.ui.results")
local runner = require("eetcode.runner")
local tabs = require("eetcode.ui.tab")
local tests = require("eetcode.ui.tests")
...
local function ready(s)
  ...
  util.err("no problem is open — use :EetCode to pick one")
  return nil
end
...
local function get_image()
  ...
  if not pcall(image.clear, "eetcode-setup-probe") then
    ...
    if not setup_ok or not pcall(image.clear, "eetcode-setup-probe") then
      image_mod = false
      return nil
    end
  end
...
function M.push()
  ...
  if not s then
    return util.err("no problem is open — use :EetCode to pick one")
  end
  save(s)
...
local function ensure_watchers()
  ...
  watched = true
  local group = vim.api.nvim_create_augroup("EetCodeProblemLifecycle", { clear = true })
...
local function keymaps(s)
  ...
  for _, buf in ipairs({ s.code_buf, s.desc_buf, s.res_buf }) do
    ...
    map(keys.run, M.run, "eetCode: run local tests")
    ...
    map("<C-w>c", function() M.close(s) end, "eetCode: close problem")
    map("<C-w>q", function() M.close(s) end, "eetCode: close problem")
    map("<C-w>o", function() M.close(s) end, "eetCode: close problem")
  end

  for _, buf in ipairs({ s.desc_buf, s.res_buf }) do
    vim.keymap.set("n", keys.quit, function() M.close(s) end,
      { buffer = buf, silent = true, desc = "eetCode: close problem" })
  end

  for _, lhs in ipairs({ "<CR>", "<Tab>" }) do
    vim.keymap.set("n", lhs, function() activate(s) end,
      { buffer = s.desc_buf, silent = true, desc = "eetCode: open hint or diagram" })
  end
end
...
local CLANGD_MARKER = "Written by eetCode.nvim"

local function support_dir()
  return config.options.solutions_dir .. "/.eetcode"
end
...
local function write_types(dir, problem_id, starter)
  local types = require("eetcode.runner.cpp").starter_types(starter)
  if #types == 0 then
    return
  end
  local body = {
...
local function build_windows(s)
  ...
  vim.bo[s.desc_buf].filetype = "eetcode-problem"
  ...
  vim.bo[s.res_buf].filetype = "eetcode-results"
