...
function M.render(opts)
  ...
  for name, pos in pairs(positions) do
    ...
    local group = complete and "EetCodeNodeDone" or "EetCodeNodeTodo"
    if opts.selected == name then
      group = "EetCodeNodeSelected"
    end
  ...
  for _, n in ipairs(node_spans) do
    ...
    span(n.bar_row, n.bar_col, n.bar_col + n.bar_filled, "EetCodeBarFill")
    span(n.bar_row, n.bar_col + n.bar_filled, n.bar_col + n.bar_total, "EetCodeBarEmpty")
  end

  for r, cols in pairs(marks) do
    for c in pairs(cols) do
      span(r, c, c + 1, "EetCodeEdge")
    end
  end
