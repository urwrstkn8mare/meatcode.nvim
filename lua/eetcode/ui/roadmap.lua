...
local dag = require("eetcode.ui.dag")
local graph = require("eetcode.catalog.graph")
...
local HIDDEN_CURSOR = "a:EetCodeHiddenCursor"
...
local function hide_cursor()
  ...
  vim.api.nvim_set_hl(0, "EetCodeHiddenCursor", { blend = 100, nocombine = true })
...
local function render()
  ...
  if not catalog.get() then
    ...
    hl.apply(state.buf, { { 2, 0, #lines[3], "EetCodeMuted" } })
    return
  end
  ...
  local spans = {
    { 0, 0, #lines[1], "EetCodeHeader" },
    { 1, 0, #lines[2], "EetCodeMuted" },
  }
...
function M.close()
  show_cursor()
  pcall(vim.api.nvim_del_augroup_by_name, "EetCodeRoadmapCursor")
  if state.tab then
...
local function cursor_autocmds()
  local group = vim.api.nvim_create_augroup("EetCodeRoadmapCursor", { clear = true })
  ...
  -- cursor and drop stale handles so the next :EetCode can reopen cleanly.
...
local function keymaps()
  ...
  map(keys.open, function()
    require("eetcode.ui.problems").open(state.selected, config.options.list)
  end, "open topic")
...
function M.open()
  ...
  vim.bo[state.buf].filetype = "eetcode-roadmap"
