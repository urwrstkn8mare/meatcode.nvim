...
function M.set(tab, name)
  ...
  local ok, prev = pcall(vim.api.nvim_tabpage_get_var, tab, "eetcode_prev_name")
  if not ok or prev == nil or prev == vim.NIL then
    local had, existing = pcall(vim.api.nvim_tabpage_get_var, tab, "name")
    set_var(tab, "eetcode_prev_name", (had and existing and existing ~= vim.NIL) and existing or "")
  end
  set_var(tab, "eetcode_name", name)
...
function M.clear(tab)
  ...
  local ok, prev = pcall(vim.api.nvim_tabpage_get_var, tab, "eetcode_prev_name")
  del_var(tab, "eetcode_name")
  del_var(tab, "eetcode_prev_name")
...
function M.draw()
  local ok, result = pcall(function()
    ...
    for i = 1, vim.fn.tabpagenr("$") do
      ...
      local label = get_var(i, "eetcode_name") or default_label(i)
