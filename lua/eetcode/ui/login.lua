...
function M.open(snippet, finish)
  ...
  vim.keymap.set("n", "y", function()
    ...
    if vim.fn.has("clipboard") == 1 then
      vim.fn.setreg("+", vim.trim(snippet))
      require("eetcode.util").notify("login script copied to clipboard")
    else
      require("eetcode.util").notify("script yanked; no clipboard provider available — use the storage steps above")
    end
