...
local TABLINE = "%!v:lua.require'meatcode.ui.tab'.draw()"

local function ensure_tabline()
  if vim.g.meatcode_tabline then
    return
  end
...
function M.name_buffer(buf, name)
  ...
  local label = "MeatCode://" .. name:gsub("[/\\]", " · ")
...
function M.set(tab, name)
  ...
  local ok, prev = pcall(vim.api.nvim_tabpage_get_var, tab, "meatcode_prev_name")
  if not ok or prev == nil or prev == vim.NIL then
    local had, existing = pcall(vim.api.nvim_tabpage_get_var, tab, "name")
    set_var(tab, "meatcode_prev_name", (had and existing and existing ~= vim.NIL) and existing or "")
  end
  set_var(tab, "meatcode_name", name)
...
function M.clear(tab)
  ...
  local ok, prev = pcall(vim.api.nvim_tabpage_get_var, tab, "meatcode_prev_name")
  del_var(tab, "meatcode_name")
  del_var(tab, "meatcode_prev_name")
...
function M.draw()
  local ok, result = pcall(function()
    ...
    for i = 1, vim.fn.tabpagenr("$") do
      ...
      local label = get_var(i, "meatcode_name") or default_label(i)
