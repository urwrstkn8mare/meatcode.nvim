...
function M.render(opts)
  ...
  for name, pos in pairs(positions) do
    ...
    local group = complete and "MeatCodeNodeDone" or "MeatCodeNodeTodo"
    if opts.selected == name then
      group = "MeatCodeNodeSelected"
    end
  ...
  for _, n in ipairs(node_spans) do
    ...
    span(n.bar_row, n.bar_col, n.bar_col + n.bar_filled, "MeatCodeBarFill")
    span(n.bar_row, n.bar_col + n.bar_filled, n.bar_col + n.bar_total, "MeatCodeBarEmpty")
  end

  for r, cols in pairs(marks) do
    for c in pairs(cols) do
      span(r, c, c + 1, "MeatCodeEdge")
    end
  end
